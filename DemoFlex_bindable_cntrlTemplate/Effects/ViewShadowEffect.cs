...
namespace DemoFlex_bindable_cntrlTemplate.Effects
{
    ...
    public static class ShadowEffect
    {
        ...
        static void OnHasShadowChanged(BindableObject bindable, object oldValue, object newValue)
        {
            ...
            if (hasShadow)
            {
                view.Effects.Add(new ViewShadowEffect());
            }
            else
            {
                var toRemove = view.Effects.FirstOrDefault(e => e is ViewShadowEffect);
                view.Effects.Clear();
                SetRadius(view, 0);
                SetDistanceX(view, 0);
                SetDistanceY(view, 0);
        ...
        {
            public ViewShadowEffect() : base($"DemoFlex_bindable_cntrlTemplate.{nameof(ViewShadowEffect)}")
            {
            }
        }
    }
